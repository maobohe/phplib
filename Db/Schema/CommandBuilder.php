...
class CommandBuilder
{
    ...
    /**
     * @return Db\Connection database connection.
     */
     ...
     * @param mixed $table the table schema ({@link Lib_Db_Schema_Table}) or the table name (string).
     ...
     * @param TableSchema $table the table schema ({@link Lib_Db_Schema_Table}) or the table name (string).
     * @param Criteria $criteria the query criteria
     * @param string $alias the alias name of the primary table. Defaults to 't'.
     * @return Db\Command query command.
     */
     ...
     * @param mixed $table the table schema ({@link Lib_Db_Schema_Table}) or the table name (string).
     * @param Criteria $criteria the query criteria
     * @return Db\Command delete command.
     */
     ...
     * @param TableSchema $table or the table name (string).
     ...
     * @return Db\Command insert command
     */
     ...
     * @param Criteria $criteria the query criteria
     * @return Db\Command update command.
     */
    public function createUpdateCommand($table, $data, $criteria)
    {
        ...
        if ($fields === array())
            throw new \PDOException(sprintf('表 "%s" 没有列被更新.', $table->name));
     ...
     * @param Criteria $criteria the query criteria
     * @return Db\Command the created command
     */
     ...
     * @param Db\Command $command database command
     ...
     * If an array, it is treated as the initial values for constructing a {@link Lib_Db_Schema_Criteria} object;
     * Otherwise, it should be an instance of {@link Lib_Db_Schema_Criteria}.
     * @param array $params parameters to be bound to an SQL statement.
     * This is only used when the first parameter is a string (query condition).
     * In other cases, please use {@link Lib_Db_Schema_Criteria::params} to set parameters.
     ...
     * If an array, it is treated as the initial values for constructing a {@link Lib_Db_Schema_Criteria};
     * Otherwise, it should be an instance of {@link Lib_Db_Schema_Criteria}.
     * @param array $params parameters to be bound to an SQL statement.
    ...
    public function createColumnCriteria($table, $columns, $condition = '', $params = array(), $prefix = null)
    {
        ...
        foreach ($columns as $name => $value) {
            if (($column = $table->getColumn($name)) !== null) {
                ...
                throw new \PDOException(sprintf('表 %s 没有名为"%s"的列.', $table->name, $name));
        }
    ...
    public function createInCondition($table, $columnName, $values, $prefix = null)
    {
        ...
        if (is_string($columnName)) // simple key
        {
            if (!isset($table->columns[$columnName]))
                throw new \PDOException(sprintf('表 "{table}" 没有名为 "{column}" 的列.', $table->name, $columnName));
        ...
        } else if (is_array($columnName)) // composite key: $values=array(array('pk1'=>'v1','pk2'=>'v2'),array(...))
        {
            foreach ($columnName as $name) {
                ...
                for ($i = 0; $i < $n; ++$i) {
                    if (isset($values[$i][$name])) {
                        ...
                        throw new \PDOException(sprintf('列 "%s" 在表 "%s" 中不存在.', $table->name, $name));
                }
            }
            if (count($values) === 1) {
                $entries = array();
            ...
            throw new \PDOException('列名必须是字符串或者数组');
    }
